...
def sync():
	...
	if files == "empty":
		...
		files = files.split()
	# files = os.popen("ls User1/").read().split()
	for file in files:
...
def download_udp(filename):
	...
	print "permissions of file " + filename + " in user1 folder is " + str(perm)
	# a = os.popen("chmod " + str(perm) + " " + filename).read()
	os.chmod(filename, int(perm,8))
	print "set the permissions of file " + filename + " in user2 folder to " + str(perm)
...
def intermediate_download_from_server ( protocol = "UDP" ):
	...
	if recievingFile == "directory":
		...
		if os.path.exists(filename):
			print "file " + filename + " exists in user2 folder"

			tcp_socket.send("exists")
			time.sleep(2)
			md5sum1 = tcp_socket.recv(1024).split()

			print "md5sum of file " + filename + " in user1 folder is " + md5sum1[0]

			md5sum2 = os.popen("md5sum " + filename).read().split()

			print "md5sum of file " + filename + " in user2 folder is " + md5sum2[0]
			...
			if md5sum1[0] != md5sum2[0]:
				...
				print "modified time of file "+ filename +" in user1 folder is " + mtime1

				mtime2 = os.path.getmtime(filename)
				print "modified time of file "+ filename +" in user2 folder is " + str(mtime2)
				...
				print "contents of file " + filename + " in both user1 folder and user2 folder are same"
				tcp_socket.send("stop")
				print "stopped downloading"
				time.sleep(2)
		else:
			print "file " + filename + " does not exists in user2 folder"
