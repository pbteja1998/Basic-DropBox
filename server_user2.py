...
def short_list(start_timestamp = "2000-01-01 00:00:00.000000000 +0530", end_timestamp = "3000-01-01 00:00:00.000000000 +0530"):
	command = "find User2 -type f -newermt '" + start_timestamp + "' ! -newermt '"+ end_timestamp + "'"
...
def intermediate_download_from_server(filename, protocol = "UDP"):
	...
	if os.path.isdir ( filename):
		...
		if existence == "exists":
			...
			if status == "continue":
				#print "contents of file " + filename + " in both user1 and user2 folder are not same"
				...
				if status == "continue":
					#print "file " + filename + " in user2 folder is latest"
				...
				# else:
					#print "file " + filename + " in user2 folder is not latest"
			# else:
				#print "contents of file " + filename + " in both user1 and user2 folder are same"
...
while True:
	...
	if args[0] == "index":
		if args[1] == "longlist":
			longlist = long_list("User2")
			conn.send(longlist)

		elif args[1] == "list":
			files = os.popen("ls User2").read()
			if files == '':
				files = "empty"
			conn.send(files)
